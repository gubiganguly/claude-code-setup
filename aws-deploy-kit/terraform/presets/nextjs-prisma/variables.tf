...
variable "run_seeds" {
  description = <<-EOT
    Run seed scripts on container boot. Default false.

    v1 ran demo seeds on every production boot, which is wrong for real data.
    Turn this on for a demo environment, leave it off for anything with users.
  EOT
  type        = bool
  default     = false
}
